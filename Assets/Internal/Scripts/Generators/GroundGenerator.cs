        ...
        for (var z = -settings.MapSizeZ; z <= settings.MapSizeZ; z++)
        {
          ...
          switch (tile.Type)
          {
            ...
            case Tile.TileType.Road:
              ...
              Instantiate(settings.RoadTiles[Random.Range(0, settings.RoadTiles.Length)], position, Quaternion.AngleAxis(tile.Rotation + settings.RoadRotationOffset, Vector3.up), parent: transform);
              break;
            case Tile.TileType.Ground:
              Debug.Log($"Instantiating ground tile at ({x}, {z})");
              Instantiate(settings.GroundTiles[Random.Range(0, settings.GroundTiles.Length)], position, Quaternion.AngleAxis(tile.Rotation, Vector3.up), parent: transform);
              break;
            case Tile.TileType.RoadCorner:
              Debug.Log($"Instantiating road corner tile at ({x}, {z}) with rotation {tile.Rotation}");
              Instantiate(settings.GroundCornerTiles[Random.Range(0, settings.GroundTiles.Length)], position, Quaternion.AngleAxis(tile.Rotation + settings.RoadCornerRotationOffset, Vector3.up), parent: transform);
              break;
            case Tile.TileType.ResourceTree:
              Debug.Log($"Instantiating resource tile at ({x}, {z})");
              Instantiate(settings.ResourceTreeTiles[Random.Range(0, settings.ResourceTreeTiles.Length)], position, Quaternion.AngleAxis(tile.Rotation, Vector3.up), parent: transform);
              break;
            case Tile.TileType.ResourceStone:
              Debug.Log($"Instantiating resource tile at ({x}, {z})");
              Instantiate(settings.ResourceStoneTiles[Random.Range(0, settings.ResourceStoneTiles.Length)], position, Quaternion.AngleAxis(tile.Rotation, Vector3.up), parent: transform);
              break;
            case Tile.TileType.ResourceDiamond:
              Debug.Log($"Instantiating resource tile at ({x}, {z})");
              Instantiate(settings.ResourceDiamondTiles[Random.Range(0, settings.ResourceDiamondTiles.Length)], position, Quaternion.AngleAxis(tile.Rotation, Vector3.up), parent: transform);
              break;
            default:
